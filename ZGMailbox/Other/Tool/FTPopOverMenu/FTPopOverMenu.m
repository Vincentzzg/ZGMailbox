...
#define FTDefaultBackgroundColor            [UIColor colorWithRed:80/255.f green:80/255.f blue:80/255.f alpha:0.4f]
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    FTPopOverMenuCell *menuCell = [[FTPopOverMenuCell alloc]initWithStyle:UITableViewCellStyleDefault
                                                          reuseIdentifier:FTPopOverMenuTableViewCellIndentifier
                                                                ...
                                                                menuImage:menuImage];
    FTPopOverMenuConfiguration *configuration = [FTPopOverMenuConfiguration defaultConfiguration];
    NSString *imageName = [NSString stringWithFormat:@"%@", _menuImageArray[indexPath.row]];
    if ([FTPopOverMenu sharedInstance].selectedIndexpath == indexPath) {
        menuCell.menuNameLabel.textColor = configuration.highlightedTextColor;
        imageName = [NSString stringWithFormat:@"%@_highlighted", imageName];
    } else {
        menuCell.menuNameLabel.textColor = configuration.textColor;
    }
    menuCell.iconImageView.image = [UIImage imageNamed:imageName];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [FTPopOverMenu sharedInstance].selectedIndexpath = indexPath;
...
+ (void) showForSender:(UIView *)sender
         ...
         withMenuArray:(NSArray<NSString*> *)menuArray
            imageArray:(NSArray *)imageArray
             doneBlock:(FTPopOverMenuDoneBlock)doneBlock
          dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock {
    [[self sharedInstance] showForSender:sender inView:inView underView:underView senderFrame:CGRectNull withMenu:menuArray imageNameArray:imageArray doneBlock:doneBlock dismissBlock:dismissBlock];
}
...
- (void) showForSender:(UIView *)sender
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [self.backgroundView addSubview:self.popMenuView];
//        [[self backgroundWindow] insertSubview:self.backgroundView belowSubview:view];
        [inView insertSubview:self.backgroundView belowSubview:underView];
